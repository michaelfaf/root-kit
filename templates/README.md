...
- **DP-2 → location.** One workspace: all five at the root. Split: `Business.md`, `Goals.md` and `Home.md` in the shared space, `About-me.md` and `install-notes.md` in the private one. The shared router carries the push-back rules and routes to the business files; see IMPLEMENT's DP-2 = B checklist for the five things to settle.
